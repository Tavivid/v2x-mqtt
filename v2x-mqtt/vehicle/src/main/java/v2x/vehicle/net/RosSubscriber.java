 ...
 *
 * - Master に定期的に LOOKUP_PUBS して Publisher 一覧を取得 - 新しく見つかった Publisher に TCP
 * 接続し、長さ付きフレーム (int length + payload) を読み続ける - publisher が後から起動しても、次の lookup
 * タイミングで自動的に接続する
 */
public class RosSubscriber implements AutoCloseable {
     ...
     * Subscriber を開始する。
     *
     * - 直ちに Master に対して lookup ループ用の resolverThread を起動する - resolverThread は 1
     * 秒ごとに LOOKUP_PUBS を実行し、新規 Endpoint にだけ startReceiver() を呼ぶ
     */
    public void start() {
        if (running) {
            return;
    ...
    /**
     * 指定 Endpoint に対する受信スレッドを起動する。 接続が切れた場合は 1 秒待って自動再接続を試みる。
     */
    private void startReceiver(MasterClient.Endpoint ep) {
        Thread t = new Thread(() -> {
            while (running) {
                try (Socket sock = new Socket(ep.host, ep.port); DataInputStream in = new DataInputStream(sock.getInputStream())) {
