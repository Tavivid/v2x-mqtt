...
/**
 * MQTT relay (no roscore)
 * - PublisherTimelineLoop 相当: MqttPublisherTimelineLoop
 * - SubscriberTimelineLoop 相当: MqttSubscriberTimelineLoop
 *
 * 環境変数:
 *   MQTT_HOST / MQTT_PORT
 *   MQTT_QOS (default 0)  ※比較実験で QoS=1 にしたい場合は 1
 ...
 *   MQTT_SAVE_QUEUE (default 4096)
 *   MQTT_SAVE_THREADS (default 1)
 */
public final class MqttRelayMain {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // メインスレッドは待機
    if (pubThread != null) pubThread.join();
    if (subThread != null) subThread.join();
